...
The production userspace consists of a machine-wide TEE supplicant and a
libfprint driver. The command-line hardware test tool is documented separately
in [07-reference-client.md](07-reference-client.md) and is not used by either
...
The implemented daemon lives in the standalone
[`wrobelda/qsee-supplicant`](https://github.com/wrobelda/qsee-supplicant)
repository. It builds and installs `qsee-supplicant`, the generic
`qsee-app-loader`, systemd/OpenRC units, and hardware-free protocol and
filesystem tests.
...
The implemented `goodixqsee` driver is specifically a TEE match-on-chip driver:
...
The driver is maintained on the
[`goodix-qsee` branch of wrobelda/libfprint](https://github.com/wrobelda/libfprint/tree/goodix-qsee).
It is a normal-world protocol client and owns, for one
...
On the Xiaomi Pad 5 Pro 5G reference platform the standalone supplicant and
loader replace the harness listener and loader. The machine service has been
restarted while `gfenu` remained loaded; after re-registering its listeners the
same TA continued initialization, enumeration, authentication, enrollment, and
removal without reloading.

The libfprint driver is discovered by stock fprintd and has been exercised
through fprintd for probe, open/close, enumeration, enrollment, duplicate
rejection, known-finger verification, single-print deletion, cancellation,
finger-present/removal events, per-sample retry feedback, and matched finger-ID
reporting. The IRQ pump re-arms the TA control fields before every drain
invocation, as required by the recovered state machine.

GNOME login and unlock use the normal fprintd PAM integration. On Alpine, where
the module is packaged but not enabled automatically, GDM needs a dedicated
`/etc/pam.d/gdm-fingerprint` service containing `pam_fprintd.so`; keep
`gdm-password` separate so password and fingerprint workers remain independent.

Signed Gatekeeper HAT support is not implemented. Enrollment on
this reference platform uses the explicitly documented challenge-only token
provider. The replaceable provider boundary accepts a complete 69-byte HAT from
a future credential service without putting PIN, Keymaster, or
synthetic-password logic in this driver.
